...
func _process(delta: float) -> void:
	# Checks if the player is near the cauldron/ inside the area2D, and the player presses E
	# Displays the brewing menu and hides the inventory if opened
	if player_near == true and Input.is_physical_key_pressed(KEY_E):
		Global.potion_brewing = true
		brewing.show()
		inventory.hide()
		Global.inventory_status = false
		# Shows the first page of the brewing menu for the pruple potion and hides the blue options
